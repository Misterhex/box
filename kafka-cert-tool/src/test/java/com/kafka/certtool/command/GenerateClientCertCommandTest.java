...
class GenerateClientCertCommandTest {

    private CertificateService certificateService;
    private KeyStoreService keyStoreService;
    private GenerateClientCertCommand command;
    ...
    void setUp() throws Exception {
        ...
        command = new GenerateClientCertCommand(certificateService, keyStoreService);
    ...
    @Test
    void testCallWithRequiredParameters() throws Exception {
        ...
        when(certificateService.generateSignedCertificate(any(), any(), any(), any())).thenReturn(mockClientCert);
        ...
        verify(keyStoreService).createKeyStore(any(), any(), any(), eq("client"), eq("changeit"), anyString());
        verify(keyStoreService).createTrustStore(any(), eq("ca"), eq("changeit"), anyString());
    }

    @Test
    void testCertificateInfoWithoutSAN() throws Exception {
        KeyPair mockKeyPair = generateMockKeyPair();
        X509Certificate mockClientCert = mock(X509Certificate.class);
        ...
        setField(command, "outputDir", tempDir.toString());
        setField(command, "cn", "client-user1");
        setField(command, "validityDays", 365);
        setField(command, "keystorePassword", "changeit");
        ...
        assertEquals("client-user1", capturedInfo.cn());
        assertNull(capturedInfo.sans());
        assertEquals(365, capturedInfo.validityDays());
    }
    ...
    void testOutputDirectoryCreation() throws Exception {
        ...
        File newDir = tempDir.resolve("client-certs").toFile();
    ...
    void testCorrectFilePathsGenerated() throws Exception {
        ...
        assertTrue(keyPathCaptor.getValue().endsWith("client-key.pem"));
        assertTrue(certPathCaptor.getValue().endsWith("client-cert.pem"));
        assertTrue(keystorePathCaptor.getValue().endsWith("client.keystore.jks"));
        assertTrue(truststorePathCaptor.getValue().endsWith("client.truststore.jks"));
    }
    ...
    void testCustomKeystorePassword() throws Exception {
        ...
        setField(command, "cn", "client1");
        setField(command, "validityDays", 365);
        setField(command, "keystorePassword", "securePassword456");

        command.call();

        verify(keyStoreService).createKeyStore(any(), any(), any(), anyString(), eq("securePassword456"), anyString());
        verify(keyStoreService).createTrustStore(any(), anyString(), eq("securePassword456"), anyString());
    }

    @Test
    void testCustomValidityDays() throws Exception {
        KeyPair mockKeyPair = generateMockKeyPair();
        X509Certificate mockClientCert = mock(X509Certificate.class);
        ...
        setField(command, "outputDir", tempDir.toString());
        setField(command, "cn", "client1");
        setField(command, "validityDays", 730);
        setField(command, "keystorePassword", "changeit");

        command.call();

        CertificateInfo capturedInfo = certInfoCaptor.getValue();
        assertEquals(730, capturedInfo.validityDays());
    }
    ...
    @Test
    void testDifferentClientNames() throws Exception {
        KeyPair mockKeyPair = generateMockKeyPair();
        X509Certificate mockClientCert = mock(X509Certificate.class);
        ...
        String[] clientNames = {"client1", "user@example.com", "CN=User,OU=Engineering"};

        for (String clientName : clientNames) {
            setField(command, "outputDir", tempDir.toString());
            setField(command, "cn", clientName);
            setField(command, "validityDays", 365);
            setField(command, "keystorePassword", "changeit");
            ...
            CertificateInfo capturedInfo = certInfoCaptor.getValue();
            assertEquals(clientName, capturedInfo.cn());
        }
    }
